...
async def get_podcasts(context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in os.listdir(DOWNLOAD_DIR):
        ...
        logger.info(f"Podcast file: {path}")
        names = item.split('_', 1)
        podcast_dict = {"host": names[0], "title": names[1]}
        
        if not db.checkPodcast(podcast_dict):
            result_data = f"(Generated by model {AI.model_name})\n\n"
            logger.info(f"This podcast is not in DB, generating key point for {item}")
            # for _ in range(1): #TODO, multiple key point for result summary
            ...
            #     f.write(result_data)
            # await context.bot.send_document(chat_id=group_id, document=key_point_file, caption="Podcast Key Point is ready!")
            # os.remove(key_point_file)
        os.remove(path)
