...
def Check_api_request_limit():
  resp = requests.get("https://api.web.finmindtrade.com/v2/user_info", params={"token": finmind_token})
...
class Stock_Predictor:
    def __init__(self, api, sn, eps, interval):
        ...
        self.eps = eps
        self.warn_str = "Warning: These PER is calculated from date {}, you can modify the date to run again.".format(self.start_date)
        self.get_warning()
    ...
    def get_PER(self):
        ...
        df = self.api.taiwan_stock_per_pbr(
            stock_id=str(self.stock_number),
...
def plotly_figure_pe(sn, df, line_num):
  ...
  for i in line_list:
      fig.add_trace(go.Scatter(x=df['date'], y=df[i], name=i))
  fig.update_layout(
      xaxis_title="Dates", yaxis_title="PE",
      font=dict(
          family="Courier New, monospace",
          size=26,
          color="#7f7f7f"),
...
def mean_reversion(sn, years, line_num=5):
  ...
  expect_val_bull_2 = up_prob * (TL - price_now) - down_prob * (price_now-df['TL-3SD'][-1])
  expect_val_bear_1 = down_prob * (price_now - TL) - up_prob * (df['TL+3SD'][-1] - price_now)
  text_list.append([Msg.WARNING, "均值回歸適合使用在穩定成長的股票上，如大盤or台積電等，高速成長及景氣循環股不適用，請小心服用。"])
  text_list.append([Msg.WARNING, "偏離越多標準差越遠代表趨勢越強，請勿直接進場。"])
  text_list.append([Msg.INFO, "{} 往上的機率為: {}%, 維持在這個區間的機率為: {}%, 往下的機率為: {}%".format(sn, round(up_prob*100, 2), round(hold_prob*100, 2), round(down_prob*100, 2))])
  text_list.append([Msg.INFO, "目前股價: {}, TL價: {}".format( price_now, round(TL, 2))])

  text_list.append([Msg.INFO, "做多評估："])
  text_list.append([Msg.INFO, "期望值為: {}, 期望報酬率為: {}% (保守計算: 上檔TL，下檔歸零)".format( round(expect_val_bull_1, 2), round(expect_val_bull_1/price_now*100, 2) )])
  text_list.append([Msg.INFO, "期望值為: {}, 期望報酬率為: {}% (樂觀計算: 上檔TL，下檔-3SD)".format( round(expect_val_bull_2, 2), round(expect_val_bull_2/price_now*100, 2))])

  text_list.append([Msg.INFO, "做空評估: "])
  text_list.append([Msg.INFO, "期望值為: {}, 期望報酬率為: {}% (樂觀計算: 上檔+3SD，下檔TL)".format( round(expect_val_bear_1, 2), round(expect_val_bear_1/price_now*100, 2) )])
  for l in text_list:
    Printf(l, file=fw)
...
def crwal_estimate_eps(sn, level, offset):
  ...
  for j in search(search_str, stop=5, pause=2.0):
    ...
    try:
      if str(soup.find(attrs={"data-ga-click-item": True}).text.split('-')[0]) != str(sn):
        continue
...
def calculator(stock_number, year, sel=1, level=4, EPS=None, csvwriter=None, No=0, isOTC=False):
  csvdata = [None] * 38
  StockName = all_stock_info.loc[all_stock_info['stock_id'] == str(stock_number)].iloc[0]['stock_name']
  Printf(f"股票名稱: {StockName}", file=fw)
  csvdata[0], csvdata[1], csvdata[2] = StockName, str(stock_number), f'=STOCK(CONCAT(B{No+1},"{".two" if isOTC else ".tw"}"))'
  split_str = "=" * 100
  """
  parameter -> sel
  # 0: 今年
  ...
  Printf("股票代號:\t{},\t\t估計EPS:\t{},\t\t歷史本益比參考年數:\t{}".format(stock_number, eps, year), file=fw)
  ...
  if PER:
    ...
    for i in range(3):
      PE, Price, Rate = pe_list[i], eps * pe_list[i], (eps * pe_list[i] - price_now) / price_now * 100
      Printf(uniformat.format( pe_rate*(i+1), PE, Price, Rate), file=fw)
      csvdata[5+i*3], csvdata[6+i*3], csvdata[7+i*3] = str(PE), str(Price), f'=({Price} - C{No+1}) / C{No+1} * 100'
    uniformat = "本益比平均為:\t{:<20.2f} 推算價位為:\t{:<20.2f} 推算潛在漲幅為:\t{:.2f}%"
    PE, Price = pe_list[-1], eps * pe_list[-1]
    Printf(uniformat.format( PE, Price, (Price - price_now) / price_now * 100), file=fw)
    csvdata[14], csvdata[15], csvdata[16] = str(PE), str(Price), f'=({Price} - C{No+1}) / C{No+1} * 100'
  ...
  for i, title in enumerate(comp_list):
    ...
    Printf(uniformat.format(title, PE, Price, (Price - price_now) / price_now * 100 ), file=fw)
    csvdata[17+i*3], csvdata[18+i*3], csvdata[19+i*3] = str(PE), str(Price), f'=({Price} - C{No+1}) / C{No+1} * 100'
  if csvwriter:
...
def stock_select(etf_id, e_eps=None, csvwriter=None, Hot=None):
  ...
  # 外資買賣超、投信買賣超、公股銀行買賣超
  buy_list =["https://histock.tw/stock/three.aspx?s=b", "https://histock.tw/stock/three.aspx?s=c", "https://histock.tw/stock/broker8.aspx"]
  stock_list = []
  ...
  for sn in stock_list:
    try:
      stock_id = all_stock_info.loc[all_stock_info['stock_name'] == sn].iloc[0]['stock_id']
      if len(stock_id) < 5:
  ...
  for i, (sn, stock_id) in enumerate(stock_dict.items(), start=1):
    ...
    calculator(stock_id, year, sel, level, e_eps, csvwriter, i, (etf_id=='006201'))
    time.sleep(5)
...
if __name__ == '__main__':
  ...
  if etf_slete:
    ...
    for etf_id in etf_list:
      ...
      csvwriter.writerow(['股票名稱', '股票代號', '昨日價格', '估計EPS', '歷史PE參考年數目',
                'PE25%', 'PE25%價位', 'PE25%潛在漲幅',
                'PE50%', 'PE50%價位', 'PE50%潛在漲幅',
                'PE75%', 'PE75%價位', 'PE75%潛在漲幅',
                'PE平均', 'PE平均價位', 'PE平均潛在漲幅',
                'TL+3SD PE', 'TL+3SD價位', 'TL+3SD在漲幅',
                'TL+2SD PE', 'TL+2SD價位', 'TL+2SD在漲幅',
                'TL+SD PE', 'TL+SD價位', 'TL+SD潛在漲幅',
                'TL PE', 'TL價位', 'TL潛在漲幅',
                'TL-SD PE', 'TLL-SD價位', 'TLL-SD潛在漲幅',
                'TL-2SD PE', 'TL-2SD價位', 'TL-2SD潛在漲幅',
                'TL-3SD PE', 'T-3SD價位', 'TL-3SD潛在漲幅'
                ])
